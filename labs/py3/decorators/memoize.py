...
def memoize(function):
    ...
    cache = {}
    #define a new method with required cache feature with same signature,
    #  '*' is added to handle multiple arguments because f, g and h require different number of arguments
    # we can define three decorators one for each f, g and h with 1, 2 and 3 parameters respectively
    # variable cache store data betweem multiple function calls
    def newMethod(*key):
        if key not in cache:
